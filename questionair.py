...
async def fetch_questionset(questionset_id: int):
    ...
    db = SessionLocal()
    questionset = db.query(Questionset).filter(
        Questionset.questionset_id == questionset_id).first()
...
async def fetch_all_questionset():
    ...
    questionset = db.query(Questionset).options(
        joinedload(models.Questionset.groupquestions)).all()
    return {"questionset": questionset}
...
async def fetch_groupquestion(groupquestion_id: int):
    ...
    db = SessionLocal()
    groupquestion = db.query(GroupQuestion).filter(
        GroupQuestion.groupquestion_id == groupquestion_id).first()
...
async def fetch_all_groupquestion():
    ...
    db = SessionLocal()
    groupquestion = db.query(GroupQuestion).options(
        joinedload(models.GroupQuestion.questions)).all()
    return {"groupquestion": groupquestion}
...
async def fetch_questionoption(questionoption_id: int):
    ...
    db = SessionLocal()
    questionoption = db.query(QuestionOption).filter(
        QuestionOption.questionoption_id == questionoption_id).first()
    return {"questionoption": questionoption}
